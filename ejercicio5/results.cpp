...
int main() {
    ...
    FILE* file_in_m = fopen("resultados_m.txt","w+");

    fprintf(file_in_n_m, "n+m n_v111 n_v112 n_v121 n_v122 n_v211 n_v212 n_v221 n_v222\n");
    fprintf(file_in_n, "n n_v111 n_v112 n_v121 n_v122 n_v211 n_v212 n_v221 n_v222\n");
    fprintf(file_in_m, "m n_v111 n_v112 n_v121 n_v122 n_v211 n_v212 n_v221 n_v222\n");

    int nodos_start = 5;
    int nodos_end = 30;
    int muestras = 1;

    vector< vector<double> > resultados_n_m(8, vector<double>(1+nodos_end+(nodos_end*(nodos_end-1))/2, 0));
    vector< vector<double> > resultados_n(8, vector<double>(1+nodos_end, 0));
    vector< vector<double> > resultados_m(8, vector<double>(1+(nodos_end*(nodos_end-1))/2, 0));
    ...
    for (int index = 4; index <= (nodos_end*(nodos_end-1))/2; index++) {
        if (veces_m[index] > 4){
            fprintf(file_in_m, "%d %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f\n", index,
                ...
                resultados_m[6][index]/veces_m[index], resultados_m[7][index]/veces_m[index]);
        }
    }
