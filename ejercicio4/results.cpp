...
int main() {
    ...
    //FILE* file_in_n_m = fopen("resultadosnm.txt","w+");
    FILE* file_in_n = fopen("resultadosn.txt","w+");
    //FILE* file_in_m = fopen("resultadosm.txt","w+");
    ...
    vector< vector<double> > resultados_n_m(4, vector<double>(1+nodos_end+(nodos_end*(nodos_end-1))/2, 0));
    vector< vector<double> > resultados_n(4, vector<double>(1+nodos_end, 0));
    vector< vector<double> > resultados_m(4, vector<double>(1+(nodos_end*(nodos_end-1))/2, 0));
    ...
    for (int index = 4; index <= nodos_end; index++) {
        if (veces_n[index] > 4){
            fprintf(file_in_n, "%d %7.4f %7.4f %7.4f %7.4f\n", index,
